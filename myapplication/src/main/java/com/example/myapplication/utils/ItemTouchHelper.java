...
public class ItemTouchHelper extends RecyclerView.ItemDecoration
    ...
    private final OnItemTouchListener mOnItemTouchListener
        ...
        boolean mItemSingleClicked = false;
    ...
    public void itemClickEvent(MotionEvent event) {
        if (mSelected == null) {
            return;
        }
        View consumeEventView = mSelected.itemView;
        if (consumeEventView instanceof ViewGroup) {
            consumeEventView = findConsumeView(consumeEventView, event.getRawX(), event.getRawY());
        }
        if (consumeEventView != null) {
            //消费事件
            Log.i(TAG, "itemClickEvent: ");
            consumeEventView.performClick();
        }
    }

    //查找child view
    private View findConsumeView(View consumeEventView, float x, float y) {
        int childCount = ((ViewGroup) consumeEventView).getChildCount();
//        从最上层View开始查找。因为View group 添加View的时候把最后添加的View放在最上层
        for (int i = childCount; i >= 0; i--) {
            View child = ((ViewGroup) consumeEventView).getChildAt(i);
            if (child == null) continue;
            if (child instanceof ViewGroup) { //child 还是一个ViewGroup
                View view = findConsumeView(child, x, y);
                if (view != null) {
                    return view;
                }
            } else {
        ...
        if (isInBounds((int) x, (int) y, consumeEventView)) {
            return consumeEventView;
        }
        return null;
}

    public boolean isInBounds(int rawX, int rawY, View view) {
        int[] outLocation = new int[2];
        view.getLocationOnScreen(outLocation);
        Rect rect = new Rect(outLocation[0], outLocation[1], outLocation[0] + view.getWidth(), outLocation[1] + view.getHeight());
